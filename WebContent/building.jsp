...
<html prefix="og: http://ogp.me/ns#" lang="it-IT" dir="ltr">
	<head>
		...
		<link rel="stylesheet" type="text/css" href="pcss/building.css?14-0-6-1-637181015076408448" media="screen,print" />
	...
	<body>
		<div id="imPageExtContainer">
			<div id="imPageIntContainer">
				...
				<div id="imPage">
					<div id="imHeader">
						<h1 class="imHidden">building - SitoAsta</h1>
...
$(function () {$('#imStickyBar_imMenuObject_03 ul li').each(function () {    var $this = $(this), timeout = 0, subtimeout = 0, width = 'none', height = 'none';        var submenu = $this.children('ul').add($this.find('.multiple-column > ul'));    $this.on('mouseenter', function () {        if($(this).parents('#imStickyBar_imMenuObject_03-menu-opened').length > 0) return;         clearTimeout(timeout);        clearTimeout(subtimeout);        $this.children('.multiple-column').show(0);        submenu.stop(false, false);        if (width == 'none') {             width = submenu.width();        }        if (height == 'none') {            height = submenu.height();            submenu.css({ overflow : 'hidden', height: 0});        }        setTimeout(function () {         submenu.css({ overflow : 'hidden'}).fadeIn(1).animate({ height: height }, 300, null, function() {$(this).css('overflow', 'visible'); });        }, 250);    }).on('mouseleave', function () {        if($(this).parents('#imStickyBar_imMenuObject_03-menu-opened').length > 0) return;         timeout = setTimeout(function () {         submenu.stop(false, false);            submenu.css('overflow', 'hidden').animate({ height: 0 }, 300, null, function() {$(this).fadeOut(0); });            subtimeout = setTimeout(function () { $this.children('.multiple-column').hide(0); }, 300);        }, 250);    });});});
				...
				<div id="imHTMLObject_3_01" class="imHTMLObject" style="height: 350px; overflow: auto; text-align: center;">
				<c:if test="${form != 4}">
				       <c:if test="${form == 1}">
				        <form action="<%="/fantauction.it/auction?key=form1"%>" class="form" method="post" >
							...
							<input type="submit" class="form-submit" id="submitButton" value="Avanti">
					    </form>
					   </c:if>
					   <c:if test="${form == 2}">
				        <form action="<%="/fantauction.it/auction?key=form2"%>" class="form" method="post" >
							<div class="form"> 
							 	<c:forEach var="squadre" items="${squadre}">
				        				<input id="nome${squadre.getId()}" name="nome${squadre.getId()}" placeholder="NomeSquadra ${squadre.getId()}" class="form-text" required type="text">
				        				<input id="crediti${squadre.getId()}" name="crediti${squadre.getId()}" placeholder="CreditiSquadre ${squadre.getId()}" class="form-text" required type="number">
				        				<br>
				        		</c:forEach>
							    <input type="submit" class="form-submit" id="submitButton" value="Avanti">
							 </div>   
					...
					<p>${summary}</p>
				<c:if test="${form == 4}"> 
				       		<input id="nome" name="name_enter" placeholder="NomeAsta" class="form-text" required type="text">
				        	<input id="password" name="password_enter" placeholder="Password" class="form-text" required type="password">
				        	<input type="submit" onclick="enter()" class="form-submit" id="submitButton" value="Avanti"> 
				</c:if>
				<c:if test="${form == 3}">
				        <p>LISTA SQUADRE:</p>
				        	<c:forEach var="squadre" items="${squadre}">
