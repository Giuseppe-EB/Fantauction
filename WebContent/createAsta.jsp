...
team:hover{
	...
	<head>
		...
		<meta property="og:url" content="http://eagsatea.it/pagina-1.html" />
		...
		<link rel="stylesheet" type="text/css" href="pcss/pagina-1.css?14-0-6-1-637175487656811207" media="screen,print" />
		...
		<script type="text/javascript">
			...
			x5engine.utils.currentPagePath = 'pagina-1.html';
	...
	<body>
		<div id="imPageExtContainer">
			<div id="imPageIntContainer">
				...
				<div id="imPage">
					<div id="imHeader">
						<h1 class="imHidden">Pagina 1 - Prova</h1>
...
$(function () {$('#imStickyBar_imMenuObject_03 ul li').each(function () {    var $this = $(this), timeout = 0, subtimeout = 0, width = 'none', height = 'none';        var submenu = $this.children('ul').add($this.find('.multiple-column > ul'));    $this.on('mouseenter', function () {        if($(this).parents('#imStickyBar_imMenuObject_03-menu-opened').length > 0) return;         clearTimeout(timeout);        clearTimeout(subtimeout);        $this.children('.multiple-column').show(0);        submenu.stop(false, false);        if (width == 'none') {             width = submenu.width();        }        if (height == 'none') {            height = submenu.height();            submenu.css({ overflow : 'hidden', height: 0});        }        setTimeout(function () {         submenu.css({ overflow : 'hidden'}).fadeIn(1).animate({ height: height }, 300, null, function() {$(this).css('overflow', 'visible'); });        }, 250);    }).on('mouseleave', function () {        if($(this).parents('#imStickyBar_imMenuObject_03-menu-opened').length > 0) return;         timeout = setTimeout(function () {         submenu.stop(false, false);            submenu.css('overflow', 'hidden').animate({ height: 0 }, 300, null, function() {$(this).fadeOut(0); });            subtimeout = setTimeout(function () { $this.children('.multiple-column').hide(0); }, 300);        }, 250);    });});});
				...
				<div id="imCell_1" class="" > <div id="imCellStyleGraphics_1"></div><div id="imCellStyle_1"  data-responsive-sequence-number="1"><div id="imHTMLObject_3_01" class="imHTMLObject" style=" overflow: auto; text-align: center;"><!DOCTYPE html>
				<html lang="en">
				  <head>
				  </head>
				  <body>
				      ...
				      <c:if test="${form == 1}">
				        <form action="<%="/Prova/Servlet?key=form1"%>" method="post" class="form-horizontal">
				            <div class="form-group"> <label class="col-md-4 control-label" for="textinput">Nome
				                Asta</label>
				              <div class="col-md-4"> 
				              	<input id="name" name="name" placeholder="Asta1" class="form-control input-md"  type="text"> </div>
				            </div>
				            <div class="form-group"> <label class="col-md-4 control-label" for="passwordinput">Password</label>
				              <div class="col-md-4"> 
				              	<input id="password" name="password" placeholder="password" class="form-control input-md" type="password">
				              </div>
				              <div class="col-md-4"> <span class="help-block">facoltativo</span>
				              </div>
				            </div>
				            <div class="form-group"> <label class="col-md-4 control-label" for="textinput">Numero
				                Squadra</label>
				              <div class="col-md-4"> 
				              	<input id="nTeam" name="nTeam" placeholder="Asta1" class="form-control input-md" type="text"> 
				              		<span class="help-block"><br>
				                	</span>
				              </div>
				              <div class="col-md-4"><span class="help-block">Mettere il numero
				                  di squadre <br>
				                </span></div>
				              <div class="col-md-4"><span class="help-block">che partecipano</span>
				              </div>
				            </div>
				        ...
				        <form action="<%="/Prova/Servlet?key=form2"%>" method="post" class="form-horizontal">
				        	<fieldset>
				        		...
				        		<div class="form-group"> 
				        			<label class="col-md-4 control-label" for="textinput">NomeAsta</label>
				        			<div class="col-md-4"> 
				        			 <c:forEach var="squadre" items="${squadre}">
				        				<input id="nome${squadre.getId()}" name="nome${squadre.getId()}" placeholder="NomeSquadra ${squadre.getId()}" class="form-control input-md" required type="text">
				        				<input id="crediti${squadre.getId()}" name="crediti${squadre.getId()}" placeholder="CreditiSquadre ${squadre.getId()}" class="form-control input-md" required type="number">
				        				<br>
				        			</c:forEach>
				        			<input type="submit" class="avanti" id="submitButton" value="Avanti">	 
				        			</div>
